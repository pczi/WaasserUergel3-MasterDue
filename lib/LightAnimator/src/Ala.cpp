...
int getStep(unsigned long &t0, unsigned long &t, int v)
{
  return ((millis() - t0) % t) * v / t;
}

float getStepFloat(unsigned long &t0, unsigned long &t, float v)
{
  return ((millis() - t0) % t) * v / t;
}
...
// PCZ MODIFIED STEP CALCULATOR

int getNextStepOnBeat(unsigned long &t0, unsigned long &t, int v, bool beat)
{
  int st = getStep(t0, t, v);
  if (beat)
  {
...
int getFirstStepOnBeat(unsigned long &t0, unsigned long &t, int v, bool beat)
{
  int st = getStep(t0, t, v);
  if (beat)
  {
    // move to first animation step
    st = 0;
    t0 = millis() - ((st * t) / v);
  }
  return st;
...
int getFirstStepSetSpeedOnBeat(unsigned long &t0, unsigned long &t, int v, bool beat, long fastSpeed, long slowSpeed)
{
  int st = getStep(t0, t, v);
  if (beat)
  {
...
float getNextStepOnBeatFloat(unsigned long &t0, unsigned long &t, int v, bool beat, int increment = 0)
{
  float st = getStepFloat(t0, t, v);
  if (beat)
  {
    // move to next animation step
    if (increment == 0)
...
float getFirstStepSetSpeedOnBeatFloat(unsigned long &t0, unsigned long &t, int v, bool beat, long fastSpeed, long slowSpeed)
{
  float st = getStepFloat(t0, t, v);
  if (beat)
  {
...
//void setSpeedOnBeat(bool beat)
//{
//  if (beat) {
//    // move to next animation step
//    speed = millis() - animStartTime;
//  }
//}
